...
#            '\n\nStruct. factor (Cartesian):\n\n', self.Fc, '\n')     
    ...
    def transform_cart(self, tt, ss, P=0):
        rnk = len(tt.shape)

        if P==0:
            Sfac=1
        elif P==1 or P==-1:
            Sfac=det(ss)**int((3+P*(-1)**rnk)/2)
        else:
        ...
        return(np.einsum(*_vars)*Sfac)   #Apply correction factor Sfac (+/-1))
    
    def transform_cart_old(self, T, S, P=0):
