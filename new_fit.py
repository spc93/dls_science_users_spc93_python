...
def peak(xdat,ydat,nbgpts=1):
    ...
    return [centre, width, sumdat, height, area, m, c]
                
                
#testing old fit code
x = frange(20,30,.1)
y = np.exp(-(x-25)**2) + rand(len(x))/10
pv_c.fit(x, y, pin=[1, 25, 1, 1, 0])
figure()
plot(x, y)
plot(x, pv_c(x),'.')
print pv_c

#testing new lmfit example
...
gmodel.param_names
result =  gmodel.fit(y, x=x, cen=25, amp=1, wid=1.0)
result.plot()
print(result.fit_report())



#sophisticated constrained fit
prms = gmodel.make_params()
prms['amp'].value = 1
prms['cen'].value = 26
prms['wid'].value = 1
prms['cen'].vary = False

result =  gmodel.fit(y, x=x, params = prms)
result.plot()
print(result.fit_report())
